...
export default function ProductCategory() {
	...
	let onGetData = async (page, sortby) => {
		try {
			const { data } = await REST_API({
				...
				}&branch=${params.product.split("&")[1].slice(-1)}&page=${page}&sortby=${
					sortby ? sortby : ""
				}`,
				method: "GET",
			});
	...
	let onGetProduct = async(category, branch, page, sortby) => {
		try {
			const {data} = await REST_API({
				url: `product/sortby?category=${category}&branch=${branch}&page=${page}&sortby=${sortby? sortby : ""}`,
				method: 'GET'
			})
			setproduct(data.data)
			setselectedpage(page)
	...
	let onGetPage = async () => {
		try {
			const { data } = await REST_API({
				url: `product/pageCategory?branch=${params.product.split("&")[1].slice(-1)}&category=${
					params.product.split("&")[0].split("=")[1]
				}`,
				method: "GET",
			});
	...
	let onGetUnit = async () => {
		try {
			const { data } = await REST_API({
				url: `product/getallproduct?category=${params.product.split("&")[0].split("=")[1]}`,
				method: "GET",
			});
			console.log(data.data[0]);
			setunit(data.data[0]);
		...
		<div>
			<div className=" bg-slate-200">
				<div className="pt-[45px] max-w-screen-xl mx-auto ">
					<div className=" bg-white  rounded-lg shadow-2xl grid grid-cols-10 gap-5 px-5 mt-10 py-10">
						{category
							? category.map((value, index) => {
									return (
										<button key={index} className="space-y-2 h-max">
											<div
												onClick={() => onGetProduct(value.id,1,1)}
